...
class TradeManager:
    def __init__(self, config, position_manager, fib_calculator):
        ...
        # Initialize last_candle_below_fib. It will be properly set by main.py based on historical data.
    ...
    def check_for_trades(self, candle, five_min_data):
        ...
        This method is called with each new 5-min candle.
        """
        # Ensure enough data for WMA Fibs. FibonacciCalculator needs 65 (42 + 24 -1) periods for full WMA Fibs.
        ...
        current_price_close = candle['close'] # Use close for overall checks
    ...
    def _check_entry_conditions(self, candle, five_min_data):
        """
        Check if new position should be opened based on arming and re-cross logic.
        Strategy: Arm when low goes below threshold, enter when close re-crosses WMA Fib 0.
        """
        # Ensure WMA Fib 0 is available for the current candle
        ...
        # ARMING LOGIC: Price (current candle's low) must go below or touch the threshold
        # This means the market has shown weakness/pulled back enough.
        ...
        # RESET LOGIC: If armed, and price moves too far above WMA Fib 0, disarm
        # This prevents stale arming if the price rallies significantly without entering.
        price_to_fib_percent = (current_candle_close - wma_fib_0_level) / wma_fib_0_level
        if self.entry_switch_armed and price_to_fib_percent > self.config['trading']['fib_reset_threshold']: # Using a config for reset threshold
            self.entry_switch_armed = False
            self.logger.print_info(f"🔄 Entry switch reset - Price moved too far ({price_to_fib_percent:.2%}) above WMA Fib 0 after arming.")
            return # Stop further checks for this candle if reset, as entry won't happen.

        # ENTRY LOGIC: Only proceed if armed AND all specific entry conditions are met
        if self.entry_switch_armed and self._entry_conditions_met(candle, five_min_data, wma_fib_0_level):
            success, result = self.position_manager.add_position(current_candle_close, candle.name) # Enter at candle's close price
            if success:
                self.logger.print_info(f"🟢 Entry triggered at {current_candle_close:.8f}.")
                self.entry_switch_armed = False # Disarm after successful entry to wait for next setup
            else:
    ...
    def _entry_conditions_met(self, candle, five_min_data, wma_fib_0_level):
        """
        Determine if all specific entry conditions are met for a re-cross strategy.
        Assumes entry_switch_armed is True when this function is called.
        """
        current_candle_close = candle['close'] # Use close for re-cross confirmation
        
        # Condition 1: Current candle's CLOSE is above WMA Fib 0 (re-cross)
        price_closed_above_fib_0 = current_candle_close > wma_fib_0_level

        # Condition 2: The PREVIOUS candle was BELOW WMA Fib 0 (confirming the re-cross from below)
        # This 'self.last_candle_below_fib' should be updated by main.py for the *previous* candle.
        ...
        # Condition 3: Candle range check (to prevent entries on volatile spikes/drops)
        ...
        # Condition 4: Position spacing check (to ensure new positions are sufficiently far apart)
        # Only applies if there are existing positions.
        if self.position_manager.positions:
            # Calculate the percentage difference from the lowest existing entry.
            # We require the new entry price to be at least `position_spacing_pct` BELOW the lowest existing entry.
            ...
            # config['trading']['position_spacing_pct'] should be negative, e.g., -0.03 for 3% below
            required_spacing_pct = self.config['trading']['position_spacing_pct'] 

            if price_difference_from_lowest > required_spacing_pct: # If price is not *enough* lower
    ...
    def _check_exit_conditions(self, current_price, current_time, five_min_data):
        ...
        if (metrics['cumulative_profit_pct'] >= self.config['trading']['cumulative_profit_target'] and
            self._check_fib50_condition(five_min_data)):
            self.logger.print_info(f"📈 Cumulative profit target met ({metrics['cumulative_profit_pct']:.2%}). Closing all positions.")
        ...
        for position in self.position_manager.positions[:]: # Iterate over a copy to allow modification
    ...
    def _should_close_position(self, position, current_price, current_time, five_min_data):
        ...
        # Calculate candles elapsed based on the timestamps in five_min_data
        candles_elapsed_df = five_min_data[five_min_data.index > entry_time]
        candles_elapsed = len(candles_elapsed_df)
        
        # Calculate remaining periods for profit target 1 countdown
        ...
        # Determine current profit target based on countdown
        ...
        # Check if profit target is met AND Fib 50 condition is satisfied
        return current_profit_pct >= current_target and self._check_fib50_condition(five_min_data)

    def _check_fib50_condition(self, five_min_data):
        """
        Check if price is below WMA Fib 50 level for exit confirmation.
        Returns True if condition is met or if 'require_fib50_cross' is False.
        """
        if not self.config['trading']['require_fib50_cross']:
            return True # Condition is not required, so always True
            
        if 'wma_fib_50' not in five_min_data.columns or five_min_data['wma_fib_50'].isnull().iloc[-1]:
            self.logger.print_warning("🚨 WMA Fib 50 not available for current candle or is NaN. Cannot check exit condition.")
            return False # Cannot satisfy condition if data is missing
            
        # Check if the previous candle's close was below WMA Fib 50.
        # This confirms a cross *below* Fib 50, indicating potential downtrend/reversal.
        if len(five_min_data) < 2:
            self.logger.print_info("DEBUG: Not enough data for Fib 50 cross check (less than 2 candles).")
            return False # Need at least previous candle to check cross

        last_completed_close = five_min_data['close'].iloc[-2] # Close of the previous full candle
        wma_fib50_level = five_min_data['wma_fib_50'].iloc[-1] # Current WMA Fib 50 level

        condition_met = last_completed_close < wma_fib50_level
        self.logger.print_info(f"DEBUG: Fib 50 Condition: Last Close ({last_completed_close:.8f}) < WMA Fib 50 ({wma_fib50_level:.8f}) = {condition_met}")
        return condition_met

    def _determine_exit_type(self, position, five_min_data):
        """Determine the type of exit for a position (target_1, target_2, or cumulative)"""
        entry_time = pd.to_datetime(position['entry_time'], utc=True)
        candles_elapsed_df = five_min_data[five_min_data.index > entry_time]
        candles_elapsed = len(candles_elapsed_df)
        time_remaining = max(0, self.config['trading']['countdown_periods'] - candles_elapsed)
        
        # Note: 'cumulative_target' is handled in _check_exit_conditions before individual positions.
        # This function only distinguishes between target_1 and target_2.
